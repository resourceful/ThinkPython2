...
함수의 시작 부분에 인터페이스를 설명하는 글을 {\bf 참고
...
\end{verbatim}
%
관례상 모든 참고 문자열은 멀티라인 문자열이라고도 불리는 세 개의
...
참고 문자열은 함수가 어떤 일을 하는지 요약한다(상세 동작 설명은
...
\item[참고 문자열(docstring):] 함수 정의의 시작 부분에 작성된 함수의
  인터페이스에 대한 설명
\index{docstring}
...
내용을 갖고 있다.  이런 류의 문장들을 {\bf 복합 문장(compound
  statement)}라고 부른다.
...
연산을 {\bf 연쇄적 조건문(chained conditional)}이다.
...
문장 들여쓰기를 하면 구조가 시각적으로 명확해지긴 하지만, {\bf 중첩된
